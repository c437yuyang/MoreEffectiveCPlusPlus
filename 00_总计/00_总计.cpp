...
// 2.但没有了缺省构造函数会无法直接建立数组(堆和非堆都是)，不能Widget w[10]; //没有默认构造函数
// 3.解决方法有(参见04_01):
...
//		就算这时候你传的引用具有多态性质也是一样，而函数调用的时候会去查找虚函数。
...
// 1.当可能出现隐式类型转换的时候，可以通过重载来避免(因为隐式类型转换需要生成临时对象，而我们可能并不需要这个临时对象就能完成想要的功能)，
// 比如: Rational r1; r1 + 2.0  其实自己来实现的话，就不需要生成一个Rational(2.0)的临时对象了。
// 比如UInt类只有一个int成员的时候，重载+号运算符就可以避免+号的时候出现类型转换
// const UInt operator+(const UInt& lhs,int rhs);
// 2.C++中有一条规则:每一个重载的operator必须带有一个用户定义类型的参数，否则的话，将会影响系统预定义的操作。

//6.7 M22:尽量用运算符的赋值形式代替其单独形式
// 1.x = x - y和x-=y的区别在于，前者产生返回值，会生成临时对象，因此效率更低
// 2.自定义类型必须显式实现-=这种赋值运算符，光实现了operator-是不行的，两者无关
// 3.operator(stand-alone形式)建议根据assignment version(-=)来实现，
// 并且自己在设计类的时候，一定要两者都提供，用户用的时候才能选择
// 比如Rational a,b,c,d; result=a+b+c+d;虽然效率低一点点，但是可读性更好
...
//6.9 M24:理解虚函数、多继承、虚基类和RTTI的代价
// 1.当调用虚函数时，被执行的代码必须与调用函数的对象的动态类型相一致，编译器实现的方式是
// 通过virtual tables(vtbl)和virtual table pointers(vptr)来实现。
// 2.vtbl是一个指针数组，一个类只要有虚函数(或者继承得到)就会有vtbl，这个表里面装了所有的虚函数的函数指针，以及具体应该使用哪一个实现
// 3.每个类只需要一份vtbl即可，不同编译器的实现不一样。
// 4.有了vtbl后，还需要vptr来使得编译器能够找到vtbl，编译器通常是把vptr在每一个声明了虚函数的对象中存放一份，
// 所以vptr和vtbl都会带来代价，如果类本身就很小，代价相对就比较大，较大对象不适合放在cache或者virtual memory page中
// 5.因此使用虚函数带来的代价:(1)vtbl(2)vptr(3)放弃使用inline(inline是在编译期确定，virtual函数需要在运行期确定)
// 6.多继承带来对虚基类的需求，菱形继承里面能够防止出现成员被多份复制，但是多继承也就会带来多个vptr，并且多继承还会本身就需要(pointer to virtual base class)，
// 因为虚继承并没有实际的数据，需要查找到基类的数据。
// 7.RTTI其实也是在类的vtbl中加入一部分type_info进行存储，因此也会增加对象体积。
// 8.但是这些东西你需要使用的时候还是必须使用，编译器的实现肯定比你手工实现更加高效稳定。
...
// 5.但是这种非成员函数版本的不要声明为inline的,声明为inline，则会拷贝多份，而违背了static的定义
...
// 这种写法的话，组合和继承直接无法编译，因为无法访问析构函数，解决方法是把析构函数换成protected解决继承，把对象换成指针解决组合
// 参见27_01
...
// 参见27_02:HeapTrack类，缺点是不能用于内置类型(还是不能用于operator new[])，因为内置类型也没有this指针
// 3.禁止对象在堆中产生:
...
//7.4 M28:灵巧指针
// 1.具体实现参加28_01
// 灵巧指针的copying函数的参数不是const reference，因为会修改对象(比如unique_ptr)
// 2.灵巧指针的拷贝和赋值采用对象转移的方式，因此在作为参数的时候，除非特殊需要，一定不能传值，要传const引用
// 3.operator*()的返回值需要用引用而不是对象，返回对象的话，多态的时候会出现slicing问题，因此也就不支持虚函数了,返回引用的话效率也更高，也更符合语义(可以对*auto_ptr进行修改)
//  operator->()的返回值则只能用:一个指向某个对象的dumb pointer或者另外一个灵巧指针
// 灵巧指针判断是否为null，不能直接用 if(auto_ptr==0)等操作，因为没有这个运算符
// 因此可以重载一个operator void*(); 就能够支持各种操作，比如if(ptn) if(!ptn) if(ptn==0)等，但是这样有问题
// auto_ptr<A>==auto_ptr<B> 也是能够通过编译的的就算A和B不相关(实际上运行中一般不会相等，因为你自己的转化成void*的做法通常是取对象的地址)
// 但是这样就允许了灵巧指针在不同类型的对象上进行比较，所以很危险，最好的做法是重载一个operator!() 就解决了，以后判断就用!ptn来
// 现在的C++ stl中是用operator bool()来实现的
// 3.尽量不要提供隐式转换到dumb ptr 这样做带来很多问题，比如用户可以去delete ptn,以及导致引用计数出错，删除一个对象两次等等
// 4.通过模板成员函数，可以实现灵巧指针的父类和子类灵巧指针对象之间的相互转换,
// 并且是必须要两个指针可以相互转换(子类转换成父类比如)，才可以转换，还可以用来实现从non-const auto_ptr到const_ptr
// 5.但是出现间接基类的时候，就可能二义性，比如C:B:A,那么这时候C向B和A转换的优先级是一样的，这也是灵巧指针的缺点，没法解决。
// 6.灵巧指针里SmartPtr<T>和SmartPtr<const T>是两个完全不同类型，因此不可能转换,互相转换的话也要用上面说到的模板成员函数来实现
// 通常的做法是把SmartPtr继承自一个SmartPtrToConst<T>类

//7.5 M29 引用计数
// 参考代码
// 引用计数的实现是有代价的，适用于:
//	少量值被大量对象共享，或者是 对象的创建和销毁代价很高或者占用内存非常多。
// 参见29_01 02 03 04

//7.6 M30 代理类
// 1.代理类的用处:
//	(1)实现二维数组的[][]操作,参见代码30_01
//	(2)防止隐式类型转换，参见M5
//	(3)通过operator[]返回代理类来实现读写判断
// 2.缺点:
//	(1)用到的运算符全部都需要重载一遍，不然无法实现两次隐式转型，比如char *p=&s1[2]; //不能实现CharProxy*到char *的隐式转换
//	(2)类似的还有+=等等
//	(3)作为对象必须被构造和析构，有一定代价

//7.7 M31让函数根据一个以上的对象决定虚拟函数的调用
// 参见代码


//8.1 M32在未来时态下开发程序
// 1.用C++语言来表达设计上的约束而不是注释或者文档，比如一个类不能被继承，应该从设计上设计为sealed class而不是注释里面去让别人不能继承
// 2.处理每个类的copying函数，即使不希望有人这样调用，也要防止别人因为调用编译器提供的默认版本出错。
// 3.最小惊讶原则:自己实现的类的操作和函数有自然的语法和直观的语义，和内置类型一致，不知道怎么做的时候仿int。
// 4.只要是能被人做的，就有人这么做，要做到出现错误调用的时候仍然不会出现大问题，就是说程序要考虑非法输入。
// 5.尽量写可移植的代码。
// 6.尽可能的封装，避免虚基类，避免RTTI的if else，尽量用无名的命名空间和文件内的静态函数或对象。


//8.2 M33 将非尾端类设计为抽象类
// 参见代码

//8.3 M34 在同一程序中混合使用C和C++
// 1.名变换:C++编译器会给每个函数换一个独一无二的名字(因为有重载)，因此obj文件中的 
//	函数名和你调用的函数名不一定匹配，但是C因为没有重载，因此不会有名变换，因此要让C++程序禁用名变换，使用extern "C"
//  extern "C"{ func1(){} func2(){}} //可以用大括号括起来
// 一种同时兼容C和C++的做法:
//#ifdef __cplusplus
//extern "C" {
//#endif // __cplusplus
...
// 2.静态初始化:C++中是在main之前执行的静态对象的初始化，main结束后执行析构
// 3.C++用new和delete，C用malloc和free ，必须成对，因此
//	少用那些不在标准库中的函数，strdup比如，都不知道该用C的free还是C++的delete来释放
// 4.C++的struct如果不使用虚函数的话，和C之间就可以安全传递，使用虚函数后，存储结构会发生改变(vtbl和vptr)
