...
// 2.但没有了缺省构造函数会无法直接建立数组(堆和非堆)，
// 3.解决方法有:(1)非堆数组的话，直接initializer list里面指定每一项
//			   (2)用指针数组来解决，但是必须记得delete并且会多出来存储空间的使用
//             (3)使用placement new分配raw memory，但是必须手动调用析构函数，而不能直接delete
