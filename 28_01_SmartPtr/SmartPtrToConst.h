...
public:
	...
	~SmartPtrToConst() { if (pointee != 0) delete pointee; }
	SmartPtrToConst(const SmartPtr& rhs);
	SmartPtrToConst& operator=(const SmartPtr& rhs);
	T* operator->();
	T& operator*();
	bool operator!();
...
template<class T>
inline SmartPtrToConst & SmartPtrToConst<T>::operator=(const SmartPtrToConst & rhs)
{
	if (rhs == *this)
		return *this;
	if (constPointee != 0)
